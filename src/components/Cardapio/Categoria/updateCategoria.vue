<template>
  <v-container fluid>
    ...
    <v-list three-line subheader>
      ...
      <v-list-item>
        <v-row justify="end">
          <v-btn class="ma-5 buttonCategory white--text" @click="addCategoria"
            >Atualizar Categoria</v-btn
          >
        </v-row>
      </v-list-item>
    </v-list>
...
export default {
  ...
  methods: {
    ...mapActions(["updateCategoria", "nameUpdateCategoria"]),
    addCategoria() {
      this.object.category_name = this.category_name;
      this.updateCategoria(this.object).then((response) => {
        if (this.getResponseCategoria) {
          this.snackSucesso = true;
          this.nameUpdateCategoria(this.object).then((response) => {
  ...
  created() {
    this.category_name = this.object.category_name;
  },
};
</script>
