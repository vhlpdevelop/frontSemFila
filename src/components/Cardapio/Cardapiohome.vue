<template>
  <v-app
    style="background: rgb(25,24,10) !important;
    ...
    <KeepAlive>
      <v-row no-gutters class="align-center justify-center">
        <v-col cols="10" lg="12" md="12" sm="12" class="mt-5 mb-n6 " v-if="itemDestaques.length > 0">
          ...
          <v-responsive :aspect-ratio="16 / 9" max-height="470px" class="">
            <v-carousel cycle hide-delimiters show-arrows-on-hover>
              <template v-slot:prev="{ on, attrs }">
                <v-btn icon v-bind="attrs" v-on="on"><v-icon fab>mdi-arrow-left</v-icon></v-btn>
              </template>
              <template v-slot:next="{ on, attrs }">
                <v-btn icon v-bind="attrs" v-on="on"><v-icon fab>mdi-arrow-right</v-icon></v-btn>
              </template>
                      ...
                      <template v-if="+index + i < itemDestaques.length">
                        <v-col cols="12" lg="3" md="4" sm="6" xs="12" :key="i">
                          <v-responsive max-width="320" :aspect-ratio="16 / 9">
                            <v-card @click="toCompra(itemDestaques[+index + i])" dark elevation="12" outlined
                              class="pa-5 rounded-xl cardDestaques">
                              <v-row align="center" justify="center">

                                <div v-if="itemDestaques[+index + i].image_url !== null">
                                  <v-img contain :aspect-ratio="16 / 9" :src="itemDestaques[+index + i].image_url"
                                    width="220px" height="200px" class=" d-none d-sm-flex">
                                    <v-row align="start" justify="start"> </v-row>
                                  </v-img>
                                  <v-img contain :aspect-ratio="16 / 9" :src="itemDestaques[+index + i].image_url"
                                    width="220px" height="200px" class=" d-flex d-sm-none"></v-img>
                                </div>
                                <div v-else>
                                  <v-img contain :aspect-ratio="16 / 9" src="https://i.ibb.co/WH7Bj1J/empty.jpg"
                                    width="220px" height="220px" class=" d-none d-sm-flex">

                                  </v-img>
                                  <v-img contain :aspect-ratio="16 / 9" src="https://i.ibb.co/WH7Bj1J/empty.jpg"
                                    width="200px" height="200px" class=" d-flex d-sm-none"></v-img>
                                </div>
                              </v-row>
                              <v-icon v-if="itemDestaques[+index + i].discount_status" absolute top right
                                color="primary">
                                mdi-sale
                              </v-icon>
                              <v-icon x-large absolute top right color="red">mdi-fire</v-icon>
                              ...
                              <v-row align="center" justify="center">
                                <p class="mt-n6 pinkColorText">
                                  <span v-if="itemDestaques[+index + i].discount_status">
                                    R${{
                                    (parseFloat(itemDestaques[+index + i].price) -
  parseFloat(itemDestaques[+index + i].discount_value)).toFixed(2)
                                    }}
                                  </span>
                                  <span v-else>
                                    R${{
                              ...
                              <v-row align="end" justify="end">
                                <v-card-actions>
                                  <v-btn rounded outlined class="pr-6 pl-6 pinkColorText"
                                    @click="toCompra(itemDestaques[+index + i])">Comprar</v-btn>
                                </v-card-actions>
                              </v-row>
                            </v-card>
        ...
        <v-col cols="10" class="" v-if="itemPromocoes.length > 0">
          ...
          <v-row align="center" justify="center" class="">
            <template v-for="(promocao, index) in itemPromocoes">
              <v-col cols="6" md="3" lg="3" sm="4" class="align-center justify-center d-flex">

                <v-card class="pa-5 flex-grow-1" elevation="5" style="background-color: rgb(251,251,255) !important">
                  <v-row align="center" justify="space-around">
                    <v-img v-if="promocao.image_url !== ''" contain :aspect-ratio="16 / 9" :src="promocao.image_url"
                      max-height="220" max-width="160" class="mt-2 d-none d-sm-flex"></v-img>
                    <v-img v-else contain :aspect-ratio="16 / 9" src="https://i.ibb.co/WH7Bj1J/empty.jpg"
                      max-height="220" max-width="160" class="mt-2 d-none d-sm-flex"></v-img>

                  </v-row>
                  <v-row align="center" justify="center">
                    <v-img v-if="promocao.image_url !== ''" contain :aspect-ratio="16 / 9" :src="promocao.image_url"
                      max-height="120" max-width="80" class="mt-2 d-flex d-sm-none"></v-img>
                    <v-img v-else contain :aspect-ratio="16 / 9" src="https://i.ibb.co/WH7Bj1J/empty.jpg"
                      max-height="120" max-width="80" class="mt-2 d-flex d-sm-none"></v-img>

                  </v-row>
                  <v-icon color="yellow" absolute top right>mdi-lightning-bolt</v-icon>
                  <v-row align="start" justify="center">
                    <v-card-title class="text-caption text-lg-body-1">
                      <span class="pinkTitle text-truncate" style="max-width: 130px">{{ promocao.item_name }}</span>
                    </v-card-title>
                  </v-row>

                  <v-row align="center" justify="center">
                    <v-subheader class="mt-n8 ">Promoção</v-subheader>
                  </v-row>
                  <v-row align="center" justify="center">
                    <v-subheader class="mt-n7 textColorDefault text-lg-button">R${{ promocao.price }}</v-subheader>
                  </v-row>
                  <v-row align="center" justify="center">
        ...
        <v-col cols="12" :key="index" class="mt-12" v-for="(categoria, index) in categorias">


          <v-container fluid fill-height class="d-flex">
            <v-row align="center" justify="center" class="mt-3">

              <v-card style="width: 100%; background-color: #19180A !important" v-if="categoria.items.length > 0" flat
                class="pa-3">
                ...
                <v-card-text>
                  <v-data-iterator :headers="headers" :items="categoria.items" hide-default-header hide-default-footer
                    disable-pagination class="d-flex flex-column mh-100 ">
                    <template v-slot:default="props">
                      <v-row class="fill-height overflow-hidden d-flex align-center justify-center" id="container">
                        <v-col v-for="(item, idx) in props.items" :key="item.item_name" :cols="12 / itemsPerRow"
                          class=" mb-5">
                          <v-card outlined class=" fill-height rounded-xl" @click="toCompra(item)">
                            <v-card-title>
                              <v-row>
                                <v-col cols="8">
                                ...
                                <v-col cols="4">
                                  <div v-if="item.image_url !== ''">
                                    <v-img class="d-none d-sm-flex" alt="Avatar" :src="item.image_url" width="180px"
                                      height="120px"></v-img>
                                    <v-img class="d-flex d-sm-none" alt="Avatar" :src="item.image_url" width="80px"
                                      height="60px"></v-img>
                                  </div>
                                  <div v-else>

                                    <v-img class="d-none d-sm-flex" alt="Avatar"
                                      src="https://i.ibb.co/WH7Bj1J/empty.jpg" width="180px" height="120px"></v-img>
                                    <v-img class="d-flex d-sm-none" alt="Avatar"
                                      src="https://i.ibb.co/WH7Bj1J/empty.jpg" width="80px" height="60px"></v-img>
                                  </div>
                                </v-col>
                              </v-row>
                            </v-card-title>
                              ...
                              <v-row no-gutters>
                                <v-col cols="8" md="10" xl="10" sm="10" lg="10">
                                  <div v-if="item.discount_status" class="ml-6">
                                    ...
                                    <b style="text-decoration: line-through;">R${{ item.price }}
                                    </b>
                                  </div>
                                  <b class="" v-else>
                                    <span class="ml-6 textColorDefault">R${{ item.price }}</span>
                                  </b>
                                </v-col>
                                <v-col cols="4" md="2" sm="2" xl="2" lg="2">
                                  ...
                                  <v-tooltip bottom v-if="item.discount_status">
                                    <template v-slot:activator="{ on, attrs }">
                                      <v-icon color="primary" dark v-bind="attrs" v-on:hover="on">
                                        mdi-sale
                                      </v-icon>
                                    </template>
